...
private OENV getOutline(qstn:singleQuestion(str qstnIdent, str qstnLabel, DataType qstnDataType), OENV nds){
	if(qstnIdent notin nds.ids){
		nds0 = addQstn(nds, qstnIdent, qstnLabel, qstnDataType, qstn@location);
		nds1 = addElemId(nds0,qstnIdent);
		return nds1;
	}
	return nds;
}	
private OENV getOutline(qstn:computableQuestion(str cqstnIdent, str cqstnLabel, DataType cqstnDataType, Expr cqstnExpr), OENV nds){
 	if(cqstnIdent notin nds.ids){
 		nds0 =  addQstn(nds, cqstnIdent, cqstnLabel, cqstnDataType, qstn@location);
 	    nds1 = addElemId(nds0,cqstnIdent);
 	    return nds1;	
 	}
 	return nds;
}
